#ifndef FILE_CHP_SEEN
#define FILE_CHP_SEEN
#include <fstream>
#include <vector>
#include <utility>
#include <iostream>
...
bool compareEdgeTypes(const EdgeType &edge1, const EdgeType &edge2)
{
    return edge1.source < edge2.source || (edge1.source == edge2.source && edge1.target < edge2.target) || (edge1.source == edge2.source && edge1.target == edge2.target && edge1.weight < edge2.weight) ;
}

bool compareEdgeTypesBackwards(const EdgeType &edge1, const EdgeType &edge2)
{
    return edge1.target < edge2.target || (edge1.target == edge2.target && edge1.source < edge2.source) || (edge1.source == edge2.source && edge1.target == edge2.target && edge1.weight < edge2.weight);
}

bool compareEdgeTypes2(const EdgeType &edge1, const EdgeType &edge2)
{
    Rank rank1 = edge1.source < edge1.target ? edge1.source : edge1.target;
    Rank rank2 = edge2.source < edge2.target ? edge2.source : edge2.target;
    return rank1 > rank2;
}
...
public:
	...
	void readFromFMIFile(string fname)
	{
		...
		vector<pair<long,Rank>> osm_rank_pairs(nofNodes);
		for (long i=0; i < nofNodes; i++)
		{
			osm_rank_pairs[i] = make_pair(nodeList[i].osmID, i);
		}
